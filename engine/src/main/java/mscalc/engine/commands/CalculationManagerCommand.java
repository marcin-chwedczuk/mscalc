...
public enum CalculationManagerCommand {
    ...
    CalculationManagerCommand(int command) {
        this.command = command;
    }

    public int toCommandInt() {
        return command;
    }

    private static Map<Integer, CalculationManagerCommand> command2Enum = new HashMap<>();

    public static CalculationManagerCommand fromCommandInt(int command) {
        synchronized (command2Enum) {
            if (command2Enum.isEmpty()) {
                for (CalculationManagerCommand enumValue : CalculationManagerCommand.values()) {
                    command2Enum.put(enumValue.toCommandInt(), enumValue);
                }
            }
